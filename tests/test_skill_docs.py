...
SKILL_ROOT = ROOT / "skills" / "openevolve-experiment-workflow"
SKILL = SKILL_ROOT / "SKILL.md"
README = SKILL_ROOT / "README.md"
README_ZH = SKILL_ROOT / "README.zh-CN.md"
PYPROJECT = ROOT / "pyproject.toml"
AGENT_META = SKILL_ROOT / "agents" / "openai.yaml"
REFERENCE_DIR = SKILL_ROOT / "references"
EXAMPLE_DIR = SKILL_ROOT / "examples" / "admm-adaptive-rho-session"
...
class SkillDocsTests(unittest.TestCase):
    ...
    def test_public_docs_are_platform_neutral_and_not_process_notes(self):
        ...
        self.assertIn("coding agent", readme_zh)
        self.assertNotIn("Codex", public_text)
        self.assertNotIn("UI bridge", readme)

    def test_readme_documents_package_install_without_adapter_surface(self):
        token = "open" + "code"
        readme = self.read(README)
        readme_zh = self.read(README_ZH)
        self.assertIn("skills/openevolve-experiment-workflow/SKILL.md", readme)
        self.assertIn("skills/openevolve-experiment-workflow/SKILL.md", readme_zh)
        self.assertNotIn(token, readme.lower())
        self.assertNotIn(token, readme_zh.lower())
    ...
    def test_examples_are_package_level_case_packages(self):
        readme = self.read(README)
        readme_zh = self.read(README_ZH)
        self.assertIn("`examples/`: package-level runnable examples", readme)
        self.assertIn("`examples/`：包内可运行示例", readme_zh)
        self.assertTrue((EXAMPLE_DIR / "README.md").is_file())
        self.assertTrue((SKILL_ROOT / "examples").is_dir())
